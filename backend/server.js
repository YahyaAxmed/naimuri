...
app.get('/api/reservation/:reservationId', (req, res) => {
    ...
    const query = 'SELECT r.id, DATE_FORMAT(booking_date, "%d") AS booking_date, DATE_FORMAT(booking_date, "%w") AS booking_day, checked_in, attendees, equipment_id, e.name as equipmentName, team_id, t.name as teamName FROM reservation as r LEFT JOIN equipment as e ON e.id = r.equipment_id LEFT JOIN team as t ON t.id = team_id WHERE r.id = ? ';
